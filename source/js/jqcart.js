...
(function($) {
  ...
  var cartData,
    ...
    totalCnt = 0,
    visibleLabel = false,
    label = $('<strong class="jqcart-subtotal__strong jqcart-subtotal__strong--label">'  + subtotal + '</strong> ' + '<span class="jqcart-subtotal__name-price"> руб</span>' + '</div>'),
    modal = '<div class="jqcart-layout"><div class="jqcart-checkout">123</div></div>',
    orderform = '<input class="jqcart-form__push" id="push" type="submit" value="Отправить заказ"><label class="push" for="push"></label></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
  ...
  var actions = {
    init: function(o) {
      ...
      $(document)
        ...
        .on('click', opts.buttons, actions.getSum)
    ...
    recalcSum: function() {
      ...
      $('.jqcart-subtotal__strong--label').text(subtotal);
      $('.jqcart-total-cnt').text(totalCnt + ' шт');
      if (totalCnt <= 0) {
				actions.hideCart();
    ...
    openCart: function() {
      ...
      cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Корзина <span class="jqcart-print-order"></span></p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div class="jqcart-thead__title">Название</div><div class="jqcart-thead__quantity">Кол-во</div><div class="jqcart-thead__price">цена</div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
          sum = Math.ceil((cartData[key].count * cartData[key].price) * 100) / 100;
          ...
          orderPreview += '<div class="jqcart-small-td jqcart-title">' + cartData[key].title + '</div>';
          orderPreview += '<div class="jqcart-quantity"><span class="jqcart-incr jqcart-incr__minus" data-incr="-1"></span><input type="text" class="jqcart-amount" value="' + cartData[key].count + '"><span class="jqcart-incr jqcart-incr__plus" data-incr="1"></span></div>';
          orderPreview += '<div class="jqcart-price">' + cartData[key].price + '</div>';
          orderPreview += '<div class="jqcart-currency">' + 'руб.' + '</div>';
          orderPreview += '<div class="jqcart-small-td"><span class="jqcart-del-item"></span></div>';
          orderPreview += '</div>';
        }
      }
      orderPreview += '</div></div>';
      orderPreview += '<div class="jqcart-subtotal"><div class="jqcart-subtotal__name">Итого</div><div class="jqcart-subtotal__total jqcart-total-cnt">' + totalCnt + ' шт' + '</div> <strong class="jqcart-subtotal__strong">'  + subtotal + '</strong> '  + '<span class="jqcart-subtotal__name-price"> руб</span>' + '</div>';
